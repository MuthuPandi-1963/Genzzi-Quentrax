...
import { CheckCircle2, XCircle, ArrowLeft, Home, Award } from "lucide-react";
...
const ResultsChart = dynamic(
  ...
  {
    loading: () => <div className="w-full h-48 bg-gray-100 dark:bg-gray-800 animate-pulse rounded-2xl flex items-center justify-center">Loading chart...</div>,
    ssr: false,
  }
);
...
export default function AssessmentResultPage() {
  ...
  const correctCount = mockResult.questions.filter(q => q.isCorrect).length;
  const incorrectCount = mockResult.questions.filter(q => !q.isCorrect && q.selected).length;
  const unansweredCount = mockResult.questions.filter(q => !q.selected).length;
    ...
    <div className="min-h-screen bg-[hsl(260,20%,98%)] dark:bg-[hsl(260,50%,4%)] pt-8 pb-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <Link href="/student/assessments" className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors font-medium">
            <ArrowLeft className="w-4 h-4" /> Back to Assessments
          </Link>
          <div className="flex items-center gap-4">
            <Link href="/student/my-assessments" className="inline-flex items-center gap-2 text-purple-600 dark:text-purple-400 hover:underline font-medium">
        ...
        <div className="mb-10">
          <ResultsChart 
            ...
            total={mockResult.questions.length} 
          />
        </div>
